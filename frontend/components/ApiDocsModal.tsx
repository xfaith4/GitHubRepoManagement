  ...
  },
  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        summary: 'Builds a refined dispatch prompt for a repo by layering operator custom instructions on top of the Phase 6 prompt-context packet. Does not rebuild the packet from scratch.',
        bodyParams: [
          { name: 'repoName', type: 'string', description: 'Repository name to build the refined prompt for (required)' },
          { name: 'roadmapPath', type: 'string', description: 'Optional explicit ROADMAP file path override' },
          { name: 'selectedItemText', type: 'string', description: 'Optional operator-forced roadmap item text override (matched against pending items)' },
          { name: 'customInstructions', type: 'string', description: 'Optional additional operator instructions appended after the generated base prompt' },
        ],
        responseFields: [
          { name: 'success', type: 'bool', description: 'Operation result flag' },
          { name: 'data.runId', type: 'string', description: 'Unique identifier for this refinement run' },
          { name: 'data.createdAt', type: 'ISO 8601', description: 'Refinement timestamp' },
          { name: 'data.selectedItemText', type: 'string', description: 'The roadmap item text selected for this refinement' },
          { name: 'data.selectionSource', type: '"value-ranked" | "roadmap-order" | "operator-selected"', description: 'How the roadmap item was selected' },
          { name: 'data.basePrompt', type: 'string', description: 'Auto-generated prompt before operator instructions' },
          { name: 'data.refinedPrompt', type: 'string', description: 'Final prompt with operator instructions appended' },
          { name: 'data.warnings', type: 'string[]', description: 'Non-fatal warnings (e.g. forced item not found in pending items)' },
          { name: 'data.packet', type: 'CopilotTaskPacket', description: 'Full underlying Phase 6 task packet that produced the base prompt' },
        ],
        notes: 'Refinement history is persisted per-repo under output/roadmap-task-history/prompt-refinements/. Retrieve with GET /api/operations/prompt/history.',
      },
      {
        method: 'GET',
        path: '/api/operations/prompt/history',
        summary: 'Returns the most recent prompt refinement records for a repository.',
        queryParams: [
