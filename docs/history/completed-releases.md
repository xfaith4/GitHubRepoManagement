...
## Release 3.5 — Trustworthy Surfaces (closed 2026-08-17, archived from ROADMAP.md)

**Release closed 2026-08-17.** The remaining residues resolved in the final
pass: the dispatch and maturity cross-endpoint equalities and the
timezone-basis walker run live in CI; the assessment/doc-readiness/value
ranking intake counts only in-scope repos; scan completion is announced via a
polite live region; and the markup-level "component computes its own metric"
tripwire was **rejected as gameable** — the live equalities are the
behavior-derived enforcement, per this repo's presence-vs-behavior principle.
The walker's first version recursed into `DateTime.Date` forever and was found
by a 315-second call-depth overflow, not by reading — the gate habit holding
to the release's last hour. Per-finding before/after:
[`docs/reference/trust-report.md`](../reference/trust-report.md).
Operator screenshots and live-portal sign-off ride Release 2.9's session;
the Operations stale-state rendering rides Release 3.2's workspace refactor.
...
- [x] **Every portfolio metric becomes an object, and no component computes one.**
...
- [x] **Make cross-view disagreement fail the build.** Add an invariant suite over
      ...
      single-basis walk.)_
- [x] **Exclude what is not the portfolio, visibly.** **Core shipped
...
- [x] **Replace every ad-hoc spinner with one async state machine.** **Model
...
- [x] **Put runner health above the fold, and make the stranded count count what
...
- [x] **One status vocabulary, two refresh verbs, and a surface that states its
...
## Closed 2026-09-14 (archived from ROADMAP.md)

### Lane 0.8 — P4 (moved verbatim, D-018 sweep)

- [x] **P4 — BOM/PS5.1 hazard (60).** Measure first: which BOM-less files
...
- [x] **3.7 / M4a follow-through — kind under steering contract 6.** `unknown`
      ...
      _(state: verified)_
      `check: pwsh ./tests/Test-KindDetection.ps1 -FailOnError`
- [x] **Lifecycle/conclusion consistency contract (steering extension 3, Rung 1).**
...
- [x] **3.7 / M4b — limiting foundation by kind applicability.** The limiting
      ...
      distribution. _(state: verified)_
...
- [x] **3.7 / M4c — next-action routing by the kind of gap.** `planning` limits
      ...
      ratio of repositories. _(state: verified)_
...
- [x] **Validator R024 — a built or verified check is a step CI runs.** 3.7 M4b
      ...
      `run:` lines. _(state: verified)_
      `check: pwsh ./tests/Test-RoadmapCheckRunsInCi.ps1 -FailOnError`

Verified 2026-09-14, each gate exiting 0 in CI on its PR head:

| Milestone | PR | Head | CI run | Gate |
| --- | --- | --- | --- | --- |
| M4a follow-through | #297 | `e447795` | 34833725263 | `Kind detection` |
| Lifecycle/conclusion consistency | #298 | `1e94a77` | 34908725867 | `Lifecycle consistency` |
| M4b | #299 | `484d93e` | 34908728788 | `Applicability` |
