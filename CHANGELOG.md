...
- **Five amendments from the 2026-08-23 external review, all accepted.** (1) An **admission rule** joins the lens in the thesis and ROADMAP §2: _one hour spent on this product must save more than one hour across the portfolio it manages_ — an item that only makes the product better at managing, validating, or describing itself is maintenance, not roadmap work. (2) The blanket guardrail "enforce L3+ roadmap maturity before any dispatch" is replaced, in the thesis and ROADMAP §8, by **a sufficient execution contract** — scope, acceptance criteria, a runnable verification — sized to repository kind and task scope; L3+ stays the default way roadmap-sourced work meets it, and Release 2.9's _two gates disagree_ item now names that as its resolution direction. (3) Release 3.6's first-interaction milestone becomes the **ranked `Today` landing** (one primary action per row, effort, orientation paragraph — deciding Lane 0.5's parked question), its _Measure_ milestone adds a **leverage family** derived from ledgers the product already keeps (finding → accepted action, action → merged improvement, operator minutes per completed task, agent first-pass success, recommendations accepted vs rejected, repositories concluded appropriate-as-is or archived), and a new milestone **defines the intentional-engineering evidence model** without scoring it. (4) **Release 3.7 — Portfolio Value Proof** is defined (`planned`): ten representative repositories chosen by kind, not conformance, each with a recorded conclusion; at least five materially improved through the product's own preview → approve → execute → validate workflow with operator minutes and outcome quality recorded; the result decides the 80+ rollout. (5) Lane 0.7's and Lane 0.8's `[x]` items and Lane 0.5's now-decided disclosure question moved **verbatim** to the archive — `ROADMAP.md` carries zero `[x]` items.
...
- `tools/Test-RoadmapStructure.ps1`: **0 errors, 0 warnings, 0 info** after the amendments — three releases found (2.9 active; 3.6 and 3.7 planned, 120 and 62 lines), `ROADMAP.md` at 899 lines with zero `[x]` items; `R010-FILE-LENGTH` and `R013-FUTURE-RELEASE-SIZE` cleared by archiving and by compressing stale narrative, not by trimming the new contracts.
...
- **"Approve and create PR task" — the wizard's last step — failed with `Cannot bind argument to parameter 'RunId' because it is an empty string`.** It has failed that way since Release 3.0 (`d4beef3`, #100). Dot-sourcing a script runs it in the **caller's** scope, and that also assigns every one of its `param()` variables there, unbound ones as `''`. `POST /api/roadmap/dispatch/execute` reached `New-RoadmapQueueEntry` by dot-sourcing `scripts\Add-RoadmapTaskToQueue.ps1` mid-request; PowerShell variable names are case-insensitive, so that script's `$RunId` landed on the `$runId` the route had minted four lines earlier and blanked it. The next statement passed the empty string to a mandatory parameter.
- **The same dot-source silently ate `$baseBranch` too.** Every queue entry the route wrote carried an empty `baseBranch` regardless of what the operator chose — and unlike the run id, that one throws nothing. `gh agent-task create --base` has no other source for it, so a dispatch simply used the wrong base.
- **The fix is structural, not a rename.** The queue contract now lives in [`backend/modules/automation/Automation.RoadmapQueue.ps1`](backend/modules/automation/Automation.RoadmapQueue.ps1), a **param-less function library** — a file with no `param()` block cannot assign anything into a caller's scope. The five functions moved **verbatim**, no signature or body edits: changing them in the same commit would have made a behaviour regression indistinguishable from the relocation. `Add-RoadmapTaskToQueue.ps1` keeps its command-line surface and its `-LoadFunctionsOnly` contract, and now dot-sources the one definition. The API host loads the library at startup alongside its other 40 libraries.
- **Grepping the pattern rather than the symptom found a second, silent instance.** `Get-StatusAdapterResult` in [`Adapters.ps1`](backend/adapters/Adapters.ps1) already knew about this hazard and hand-restored three variables after dot-sourcing `Invoke-Reconciliation.ps1` — but it shares **four** parameter names with it. The missed one is `$LogPath`, so every structured log that function emitted went to the default path instead of the caller's. Nothing failed, which is exactly why it survived.
- **A third instance was in the gate itself.** `Invoke-ModuleSmokeTest.ps1` dot-sources `Watch-PortalHealth.ps1`, which declares its own `$WorkspaceRoot`. It is benign today only because that script recomputes the same repo root from its own location — a smoke run given `-WorkspaceRoot` elsewhere would have snapped silently back to the real repo halfway through and asserted against the wrong tree. (`Invoke-Reconciliation.Modular.ps1`, checked in the same sweep, already restores all nine of its collisions and needed nothing.)
- **The route had no test on the path that was broken.** Both existing api-host assertions on `/api/roadmap/dispatch/execute` are **refusals** — quota `409`, in-process `409` — and both return before the code that enqueues. A contract proven only by its refusals is not proven. The suite now asserts the success path the wizard actually uses: HTTP 200, a **non-empty** `runId`, `branch === roadmap/<runId>`, a queue line whose `runId` matches what the caller was told, the requested `baseBranch` surviving into that line, and the `queued` run summary the runner claims on.

### Verification

- **Root cause reproduced in isolation before any fix** — a 7-line script dot-sourcing a `param($RunId)` script beside a local `$runId` prints `after: runId=[]` and then dies with the user's exact error string.
- **Module smoke exit 0**, including the new gate: `dot-source scope ok: 41 host dot-sources all param-less; 101 site(s) across 85 script(s) resolved and swept, every caller-variable collision restored`.
- **Both tripwires are derived from source, not from a maintained list.** The first asserts over the host's own AST that every script it dot-sources declares no parameters, and **fails closed** on a target it cannot resolve. The second sweeps **all 101 dot-source sites** across the 85 scripts under `backend\`, `scripts\` and `tools\`: for each, it intersects the target's parameters with the names live in the enclosing scope — that scope's parameters **plus anything assigned above the dot-source**, because the dispatch route lost a plain local, not a parameter — and requires every collision to be re-assigned below. Dot-sources inside `& { ... }` are skipped deliberately (child scope, assignments die with it); `. { ... }` is not.
- **The sweep resolves variable targets rather than skipping them.** 30 of those 101 sites name their script through a variable (`. $roadmapParser`), and an early version silently passed all 30 — a 30% hole in a gate whose only value is being exhaustive. It now resolves the variable to the literal it was assigned, and a site it still cannot resolve is a **failure**, not a pass.
- **Adversarially proven, four ways.** Re-arming the in-route dot-source fired `host line 9671 dot-sources Add-RoadmapTaskToQueue.ps1, which declares parameters [...]`. Deleting the `$LogPath` restore fired `never restores [LogPath] after dot-sourcing it (line 147)`. Removing the library from the host's load list fired `POST /api/roadmap/dispatch/execute cannot build a queue entry without it`. And run against the commit **before** this fix, the repo-wide sweep reports **four** sites — both real defects among them — where the fixed tree reports zero.
- **Api-host smoke exit 0**: `dispatch success path ok: runId=20260811-044714-60c3d12b branch=roadmap/20260811-044714-60c3d12b baseBranch=smoke-base queue+summary written`.
- **Lint gate 597/597, baseline unmoved.** Moving the functions verbatim kept their two long-standing findings (plural noun, ShouldProcess) classified as existing debt; an earlier attempt that added `[OutputType([string[]])]` to `Get-RoadmapDispatchTargets` pushed the ratchet to 598 and was reverted rather than baselined.
...
- **Live production proof of the `/api/status` tier fix** (2026-08-10 17:22, service running the fixed code since 05:59:40): a forced cold scan, `GET /api/status?refresh=true`, returned **HTTP 200 in 194s** — past the 180s deadline that previously terminated the process — with the service PID unchanged across the request and **no new `FailFast`**; the last one remains 05:45:52, before the fix loaded. The originally reported symptom is gone too: the roadmap modal's two endpoints answer immediately (`/api/roadmap-agent/history` HTTP 200 in 0s, `/api/roadmap/content` HTTP 200 in 4s) where both previously hung behind a scan that was about to kill the host.
