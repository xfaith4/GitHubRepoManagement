...
| A18 | `auto` routing is **off until D-013 is decided**. `agent-providers.json` carries `dispatch: { defaultTarget: "claude", autoEnabled: false }`. The dispatch route defaults an absent `dispatchTarget` to `dispatch.defaultTarget`; an explicit `auto` while `autoEnabled` is false is a 400 (`validation`). H38-17's last step, run only when D-013 is **Decided**, flips both values to `auto` / `true`. Remediation entries (H38-31) use `dispatch.defaultTarget` the same way. | Without this, H38-18 defaulting to `auto` plus H38-17's D-013 stop leaves every board dispatch queued with "routing not enabled" forever. Nothing may write a token the runner cannot claim. | H38-07, H38-11, H38-17, H38-18, H38-31 |
...
| D-013 | Ranking weights and the tie-break: when Codex and Claude are both eligible with equal score, which runs first — or is a tie resolved by `resetAt` (nearest reset last)? | The spec says "the exact scoring weights are configuration" and names no tie rule; this is preference. | Config ships equal weights and `tieBreak: "alphabetical"` marked provisional; the router returns `tie: true` in the reason list. **Routing stays off** (`dispatch.autoEnabled = false`, A18): every board dispatch and every remediation entry goes to `claude`, and nothing waits on the router. | H38-17 (the `auto` claim branch and the config flip only; the router function itself lands) |
...
| H38-17 | M3 | Router: eligibility then ranking, reason recorded, resolved at claim time | H38-09, H38-15, D-013, H-13b | high |
| H38-18 | M3 | One vocabulary at the host: dispatch/execute accepts a target (default from config, `claude` until D-013), approve route stops writing `operator-runner`, backlog by provider | H38-14 | medium |
...
1. Write the config with `"schemaVersion": "v1"` and this shape (values are the spec's initial policy or §B defaults, every provisional one carrying `"provisional": true`):
   ...
   ```json
   {
     ...
     "ranking":   { "weights": { "suitability": 1, "usableCapacity": 1, "history": 1, "fitsWindow": 1, "sessionReuse": 1, "timeToReset": 1, "estimatedConsumption": -1, "recentFailureRate": -1 }, "tieBreak": "alphabetical", "provisional": true },
...
3. Add D-011 … D-015 to `open-decisions.md` under **Open**, in the register's entry format (`Asked`, `Question`, `Why it is not an agent's call`, `Default if unanswered`, `Blocks`), copying §B's text.
...
- Mutations in a temp copy: `executionMode = 'cloud'`, a `unit = 'gallons'`, `shortWindowRatio = 1.5`, `localExecutionSlots = 2`, `dispatch.defaultTarget = 'auto'` with `autoEnabled = $false` → each exact error string.
- Config tripwire: the file's `schemaVersion` is the string `v1` (the AGENTS.md rule); the committed file has `dispatch.autoEnabled = $false` and `dispatch.defaultTarget = 'claude'` (H38-17 is the only packet that changes them).

**Roadmap write-back:** M2 `_(state: planned)_` → `_(state: scaffolded <date> — H38-07 agent-providers.json (schemaVersion v1) + Get-AgentProviderConfig; reserves and weights are data, marked provisional pending D-011/D-013)_`.
...
**Prerequisites:** H38-09, H38-15, **H-13b** (`Get-NextEligibleRoadmapItem` supplies the dependency clause of eligibility), **D-013 decided** (see Stop if).
...
4. **Only when D-013 is Decided** in `open-decisions.md`: set `dispatch.autoEnabled` to `true` and `dispatch.defaultTarget` to `"auto"` in `agent-providers.json` (A18), update H38-07's config tripwire to assert the new values, and copy Ben's weights and tie-break from the decision into `ranking` (drop `"provisional": true` there). This step is the switch that turns routing on; nothing else flips it.
...
**Stop if:** D-013 is not decided (do steps 1–3 with the provisional weights, **skip step 4**, leave `dispatch.autoEnabled = false` so every dispatch still goes to `claude`, and report that routing is built but off); or H-13b's selector is not merged (the dependency clause cannot be wired).
...
**Roadmap write-back:** append `; H38-18 dispatch/execute takes a target (default from config, auto refused until D-013), approval reports the real token, backlog is counted per registry token — queuedClaude/queuedCopilot unchanged`.
...
3. `executeRoadmapDispatch`: the body gains optional `dispatchTarget?: ProviderToken`; default omitted (the host defaults to the config's `dispatch.defaultTarget` — `claude` until D-013, A18).
