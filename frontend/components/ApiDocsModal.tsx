...
const CATEGORIES: CategoryDef[] = [
  ...
  },
  {
    ...
    endpoints: [
      ...
      {
        method: 'GET',
        path: '/api/trace/{runId}',
        summary: 'Release 3.1 — resolves one work item’s whole journey: rank → prompt → dispatch → agent run → Actions → merge readiness → write-back, joined from the four ledgers that each own one stage.',
        responseFields: [
          { name: 'success', type: 'bool', description: 'Operation result flag' },
          { name: 'data.keys', type: 'object', description: 'The resolved identity: packetId, packagingRunId, dispatchRunId, agentRunId' },
          { name: 'data.stages', type: 'WorkItemTraceStage[]', description: 'All seven stages, always. Each carries present, at, artifacts (the files that prove it), summary, nextAction, detail' },
          { name: 'data.currentStage', type: 'string', description: 'The furthest stage reached' },
          { name: 'data.firstGapStage', type: 'string', description: 'The first stage with no evidence — reported even when a later stage succeeded, so a hole in the middle is not hidden' },
          { name: 'data.nextAction', type: 'string', description: 'What would advance the loop from its first gap' },
        ],
        notes: 'Any of the four ids resolves the same trace. Returns 404 (with the empty seven-stage trace in data) when no stage carries the id — distinct from a trace that resolved but has stages missing.',
      },
      {
        method: 'POST',
        path: '/api/roadmap/write-back/preview',
        summary: 'Release 3.1 — proposes the roadmap completion edit for a traced work item, gated on merge evidence. Preview-first: it writes nothing and opens no PR.',
        bodyParams: [
          { name: 'traceKey', type: 'string', description: 'Any id from the trace (packet, packaging run, dispatch run, agent run). The evidence is read from the loop’s own records.' },
          { name: 'repoName', type: 'string', description: 'Required only when no traceKey resolves' },
          { name: 'itemText', type: 'string', description: 'The roadmap item to mark complete; taken from the trace when omitted' },
          { name: 'evidence', type: 'object', description: 'Explicit merge evidence for work that did not travel the packaged-item path: prUrl/prNumber, prState, mergedAt or mergeCommitSha, actionsStatus, actionsConclusion' },
          { name: 'allowMissingActions', type: 'bool', description: 'Off by default. Admits a verified merge whose Actions run was never observed — a repo with no workflows could otherwise never satisfy actions-missing. Recorded on the write-back ledger entry so the override is auditable.' },
        ],
        responseFields: [
          { name: 'data.proposedContent', type: 'string', description: 'The full roadmap with exactly one checkbox flipped and one evidence note appended' },
          { name: 'data.diff', type: 'string', description: 'Unified diff of the changed region' },
          { name: 'data.evidenceNote', type: 'string', description: 'The inserted line naming the merged PR and the validation run' },
          { name: 'data.applyRoute', type: 'string', description: 'Where to apply it: POST /api/roadmap/repair/submit-pr (createPr=true)' },
        ],
        notes: 'Refuses with 409 and category write-back-refused:<reason> — no-evidence, churn-only, no-pr, pr-not-merged, merge-unverified, actions-missing, actions-incomplete, actions-failing, ambiguous-item, already-complete, item-not-found. A refusal carries no proposedContent. Every decision, refusals included, is appended to output/roadmap-writeback.jsonl.',
      },
      {
        method: 'POST',
        path: '/api/roadmap/completion-preview',
        summary: 'Multi-item completion preview. Release 3.1 put it behind the same merge-evidence gate as /api/roadmap/write-back/preview; it used to flip any checkbox it was handed.',
        bodyParams: [
          { name: 'repoName', type: 'string', description: 'Required' },
          { name: 'completedItems', type: 'string[]', description: 'Item texts to mark complete' },
          { name: 'traceKey', type: 'string', description: 'Resolves the merge evidence from the loop’s records' },
          { name: 'evidence', type: 'object', description: 'Explicit merge evidence, as an alternative to traceKey' },
        ],
        responseFields: [
          { name: 'data.proposedContent', type: 'string', description: 'The roadmap with each matched item completed and an evidence note appended' },
          { name: 'data.markedCount', type: 'number', description: 'How many items were marked' },
          { name: 'data.refusedItems', type: 'object[]', description: 'Per-item refusals (ambiguous-item, already-complete, item-not-found) with their reasons' },
        ],
        notes: 'Returns 409 write-back-refused:<reason> when the evidence does not justify completion, and 400 without repoName.',
      },
    ],
  },
  {
