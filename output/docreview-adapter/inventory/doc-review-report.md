# Documentation Review Inventory

Generated: 2026-03-16 06:51:42

## Summary

- Repos with Markdown docs: 21
- High priority: 11
- Medium priority: 10
- Low priority: 1

### Review Modes

- **Full-doc pass:** 6
- **Core-docs batch:** 9
- **Needs taxonomy before rewrite:** 1
- **Archive cleanup first:** 1
- **README-only:** 5

## AIToolbox

- **Path:** G:\Development\20_Staging\AIToolbox
- **Priority:** High (90)
- **Review mode:** Full-doc pass
- **Docs:** 59 active / 0 archive / 0 generated (total 61)
- **Categories:** architecture, changelog, contributing, general, readme, reference, setup, troubleshooting
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** True
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-02-18 01:37:50 -0500
- **Uncommitted changes:** 5
- **Doc quality hints:** contains-risk-markers

### Key Docs

- `CONTRIBUTING.md` _Active_
- `docs\adr\0001-use-fastapi.md` _Active_
- `docs\adr\0005-multi-agent-architecture.md` _Active_
- `docs\api-reference.md` _Reference_
- `docs\architecture.md` _Active_
- `docs\contributing.md` _Active_
- `docs\development\architecture-deep-dive.md` _Active_
- `docs\development\setup.md` _Active_
- `docs\FAQ.md` _Active_
- `docs\mcp\README.md` _Active_
- `docs\PromptAsCode\PROMPT_CHAIN_QUICK_REFERENCE.md` _Active_
- `docs\README.md` _Active_
- `README.md` _Active_

## SystemDashboard

- **Path:** G:\Development\20_Staging\SystemDashboard
- **Priority:** High (90)
- **Review mode:** Full-doc pass
- **Docs:** 53 active / 5 archive / 0 generated (total 59)
- **Categories:** architecture, changelog, contributing, general, readme, reference, roadmap, setup, troubleshooting
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** True
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-02-14 01:34:31 -0500
- **Uncommitted changes:** 97
- **Doc quality hints:** contains-risk-markers, duplicate-setup-docs

### Key Docs

- `docs\API-REFERENCE.md` _Reference_
- `docs\ARCHITECTURE.md` _Active_
- `docs\CONTRIBUTING.md` _Active_
- `docs\FAQ.md` _Active_
- `docs\observability-pack-v1-design.md` _Active_
- `docs\README.md` _Active_
- `docs\ROADMAP-QUICK-REFERENCE.md` _Active_
- `docs\screenshots\README.md` _Active_
- `docs\SECURITY-SETUP.md` _Active_
- `docs\SETUP.md` _Active_
- `docs\TROUBLESHOOTING.md` _Active_
- `migrations\README.md` _Active_
- `README.md` _Active_

## UnifiedAIToolbox

- **Path:** G:\Development\20_Staging\UnifiedAIToolbox
- **Priority:** High (70)
- **Review mode:** Full-doc pass
- **Docs:** 52 active / 30 archive / 17 generated (total 101)
- **Categories:** architecture, changelog, contributing, docs-index, general, readme, roadmap, runbook, setup
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** True
- **Has docs index:** True
- **Git branch:** main
- **Last commit:** 2026-03-14 02:36:12 -0400
- **Uncommitted changes:** 3
- **Doc quality hints:** contains-risk-markers

### Key Docs

- `apps\orchestration-bridge\MERGE_COORDINATOR_RUNBOOK.md` _Operational_
- `apps\orchestration-bridge\README.md` _Active_
- `apps\orchestration-bridge\TASK_EXECUTOR_RUNBOOK.md` _Operational_
- `apps\unifiedtoolbox.webapp\README.md` _Active_
- `docs\index.md` _Active_
- `docs\mcp\README.md` _Active_
- `docs\README.md` _Active_
- `docs\Unified-AI-Toolbox-Architecture.md` _Active_
- `modules\GitHubRepoManager\README.md` _Active_
- `Orchestration\Goals\README.md` _Active_
- `Orchestration\README.md` _Active_
- `packages\prompt-registry\README.md` _Active_
- `project files\branding\README.md` _Active_
- `prompts\README.md` _Active_
- `README.md` _Active_
- `scripts\README.md` _Active_
- `scripts\swarms\CONTRIBUTING.md` _Active_
- `scripts\swarms\README.md` _Active_
- `templates\ci-cd-blueprint\README.md` _Active_
- `Unified-AI-Toolbox-Architecture.md` _Active_

## GenesysCloudAuditor

- **Path:** G:\Development\20_Staging\GenesysCloudAuditor
- **Priority:** High (70)
- **Review mode:** Full-doc pass
- **Docs:** 24 active / 0 archive / 11 generated (total 35)
- **Categories:** architecture, general, readme, reference, roadmap, setup
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-03-16 03:48:56 -0400
- **Uncommitted changes:** 0
- **Doc quality hints:** vague-md-names, contains-risk-markers

### Key Docs

- `docs\application-architecture.md` _Active_
- `docs\architecture.md` _Active_
- `docs\oauth-and-api-resilience.md` _Active_
- `docs\README.md` _Active_
- `docs\setup-and-operations.md` _Active_
- `QuickStart.md` _Active_
- `README.md` _Active_

## ElasticSearch-1

- **Path:** G:\Development\20_Staging\ElasticSearch-1
- **Priority:** High (65)
- **Review mode:** Full-doc pass
- **Docs:** 11 active / 1 archive / 0 generated (total 14)
- **Categories:** general, readme, setup, troubleshooting
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-02-26 12:27:00 -0500
- **Uncommitted changes:** 0

### Key Docs

- `agents\genesys_troubleshooter\README.md` _Active_
- `agents\genesys_troubleshooter\system_prompt.md` _Active_
- `agents\platform_monitor\README.md` _Active_
- `agents\snow_incident_formatter\README.md` _Operational_
- `docs\02_agent_setup.md` _Active_
- `kibana\README.md` _Active_
- `README.md` _Active_

## FowlingScorecard

- **Path:** G:\Development\20_Staging\FowlingScorecard
- **Priority:** High (55)
- **Review mode:** Full-doc pass
- **Docs:** 19 active / 0 archive / 2 generated (total 22)
- **Categories:** general, readme, runbook, setup
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-02-27 00:01:31 -0500
- **Uncommitted changes:** 3
- **Doc quality hints:** contains-risk-markers

### Key Docs

- `docs\README.md` _Active_
- `prototype-v1\README.md` _Active_
- `QUICKSTART.md` _Active_
- `README.md` _Active_
- `THURSDAY_LEAGUE_PLAYBOOK.md` _Operational_

## SampleDashboardReact

- **Path:** G:\Development\20_Staging\SampleDashboardReact
- **Priority:** High (45)
- **Review mode:** Core-docs batch
- **Docs:** 3 active / 0 archive / 0 generated (total 3)
- **Categories:** general, readme
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2025-11-04 19:38:48 -0500
- **Uncommitted changes:** 2

### Key Docs

- `README.md` _Active_

## AI-Orchestration

- **Path:** G:\Development\20_Staging\AI-Orchestration
- **Priority:** High (40)
- **Review mode:** Needs taxonomy before rewrite
- **Docs:** 19 active / 0 archive / 0 generated (total 19)
- **Categories:** general, readme, setup
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** React/Vite-Dashboard
- **Last commit:** 2025-10-26 22:29:50 -0400
- **Uncommitted changes:** 31
- **Doc quality hints:** no-docs-folder-many-md, duplicate-setup-docs

### Key Docs

- `DEPLOYMENT_QUICKSTART.md` _Active_
- `GITHUB_PAGES_SETUP.md` _Active_
- `MilestoneDashboard\README.md` _Active_
- `QUICKSTART.md` _Active_
- `README.md` _Active_

## SereneHarmony_Site_Starter

- **Path:** G:\Development\20_Staging\SereneHarmony_Site_Starter
- **Priority:** High (40)
- **Review mode:** Core-docs batch
- **Docs:** 8 active / 0 archive / 0 generated (total 8)
- **Categories:** general, readme, setup
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-03-12 18:28:27 -0400
- **Uncommitted changes:** 0
- **Doc quality hints:** no-docs-folder-many-md

### Key Docs

- `custom_SereneHarmonySite\FORM_SETUP.md` _Active_
- `custom_SereneHarmonySite\README.md` _Active_
- `README.md` _Active_

## AI_PromptRefiner_GUI

- **Path:** G:\Development\20_Staging\AI_PromptRefiner_GUI
- **Priority:** High (40)
- **Review mode:** Core-docs batch
- **Docs:** 5 active / 0 archive / 0 generated (total 5)
- **Categories:** changelog, general, readme, setup
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2025-09-27 12:40:28 -0400
- **Uncommitted changes:** 2

### Key Docs

- `GITHUB_SETUP.md` _Active_
- `README.md` _Active_

## PromptRefiner_GUI

- **Path:** G:\Development\20_Staging\PromptRefiner_GUI
- **Priority:** High (40)
- **Review mode:** Core-docs batch
- **Docs:** 5 active / 0 archive / 0 generated (total 5)
- **Categories:** changelog, general, readme, setup
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2025-09-27 12:40:28 -0400
- **Uncommitted changes:** 2

### Key Docs

- `GITHUB_SETUP.md` _Active_
- `README.md` _Active_

## OpenAI_Refiner

- **Path:** G:\Development\20_Staging\OpenAI_Refiner
- **Priority:** Medium (35)
- **Review mode:** README-only
- **Docs:** 1 active / 0 archive / 0 generated (total 1)
- **Categories:** readme
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-02-10 00:59:38 -0500
- **Uncommitted changes:** 3

### Key Docs

- `README.md` _Active_

## OpsWorkbench

- **Path:** G:\Development\20_Staging\OpsWorkbench
- **Priority:** Medium (35)
- **Review mode:** README-only
- **Docs:** 1 active / 0 archive / 0 generated (total 1)
- **Categories:** readme
- **Has README:** True
- **Has docs folder:** True
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2025-12-25 20:43:40 -0500
- **Uncommitted changes:** 1

### Key Docs

- `README.md` _Active_

## 2026MiddleEastWar

- **Path:** G:\Development\20_Staging\2026MiddleEastWar
- **Priority:** Medium (30)
- **Review mode:** Core-docs batch
- **Docs:** 8 active / 0 archive / 0 generated (total 9)
- **Categories:** general, readme, runbook
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-03-07 21:51:53 -0500
- **Uncommitted changes:** 0
- **Doc quality hints:** no-docs-folder-many-md

### Key Docs

- `logs\README.md` _Active_
- `outputs\neighborhood_runbook.md` _Operational_
- `README.md` _Active_

## quickstart-resources

- **Path:** G:\Development\20_Staging\quickstart-resources
- **Priority:** Medium (30)
- **Review mode:** Core-docs batch
- **Docs:** 6 active / 0 archive / 0 generated (total 6)
- **Categories:** readme
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2025-10-07 14:13:48 +0100
- **Uncommitted changes:** 2
- **Doc quality hints:** no-docs-folder-many-md

### Key Docs

- `mcp-client-python\README.md` _Active_
- `mcp-client-typescript\README.md` _Active_
- `README.md` _Active_
- `weather-server-python\README.md` _Active_
- `weather-server-rust\README.md` _Active_
- `weather-server-typescript\README.md` _Active_

## Ideal-Prompt-Library

- **Path:** G:\Development\20_Staging\Ideal-Prompt-Library
- **Priority:** Medium (30)
- **Review mode:** Core-docs batch
- **Docs:** 5 active / 0 archive / 0 generated (total 5)
- **Categories:** changelog, general, readme
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-03-08 23:24:29 -0400
- **Uncommitted changes:** 2

### Key Docs

- `README.md` _Active_

## DataExtraction

- **Path:** G:\Development\20_Staging\DataExtraction
- **Priority:** Medium (30)
- **Review mode:** Core-docs batch
- **Docs:** 4 active / 0 archive / 0 generated (total 4)
- **Categories:** general, readme
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2025-09-27 11:29:41 -0400
- **Uncommitted changes:** 0

### Key Docs

- `README.md` _Active_

## GeminiAIOrchestrator

- **Path:** G:\Development\20_Staging\GeminiAIOrchestrator
- **Priority:** Medium (30)
- **Review mode:** Core-docs batch
- **Docs:** 3 active / 0 archive / 0 generated (total 3)
- **Categories:** general, readme
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2025-10-25 20:46:35 -0400
- **Uncommitted changes:** 0

### Key Docs

- `README.md` _Active_

## 2025-09-11-System-Dashboard

- **Path:** G:\Development\20_Staging\2025-09-11-System-Dashboard
- **Priority:** Medium (20)
- **Review mode:** README-only
- **Docs:** 1 active / 0 archive / 0 generated (total 1)
- **Categories:** readme
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-03-09 04:01:14 -0400
- **Uncommitted changes:** 1

### Key Docs

- `README.md` _Active_

## NetworkTracker

- **Path:** G:\Development\20_Staging\NetworkTracker
- **Priority:** Medium (20)
- **Review mode:** README-only
- **Docs:** 1 active / 0 archive / 0 generated (total 1)
- **Categories:** readme
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2026-02-22 07:30:13 -0500
- **Uncommitted changes:** 1

### Key Docs

- `README.md` _Active_

## SnapGoal

- **Path:** G:\Development\20_Staging\SnapGoal
- **Priority:** Medium (20)
- **Review mode:** README-only
- **Docs:** 1 active / 0 archive / 0 generated (total 1)
- **Categories:** readme
- **Has README:** True
- **Has docs folder:** False
- **Has contributing guide:** False
- **Has docs index:** False
- **Git branch:** main
- **Last commit:** 2024-03-14 23:28:56 -0400
- **Uncommitted changes:** 0

### Key Docs

- `README.md` _Active_

