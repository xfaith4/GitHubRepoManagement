...
function normalizeRepo(repo: any): RepoStatus {
  ...
  return {
    ...
    // Release 3.1 — carried through verbatim rather than re-derived here, so the
    // dashboard reports the basis the backend actually used instead of a second
    // opinion computed from a different pair of fields.
    staleness: repo?.staleness
      ? {
          state: String(repo.staleness.state ?? 'unknown') as RepoStaleness['state'],
          isStale: Boolean(repo.staleness.isStale ?? false),
          basis: String(repo.staleness.basis ?? ''),
          localCommitDate: repo.staleness.localCommitDate ?? null,
          remotePushedAt: repo.staleness.remotePushedAt ?? null,
          driftSeconds: repo.staleness.driftSeconds ?? null,
          behindByDays: repo.staleness.behindByDays ?? null,
          toleranceSeconds: Number(repo.staleness.toleranceSeconds ?? 0),
          exactCountsAvailable: Boolean(repo.staleness.exactCountsAvailable ?? false),
          summary: String(repo.staleness.summary ?? ''),
        }
      : null,
...
function normalizeOperationsRepoEntry(entry: any): OperationsRepoEntry {
  return {
    ...
    pendingCount: Number(entry?.pendingCount ?? entry?.pendingItemCount ?? 0),
    nextPendingItem: entry?.nextPendingItem ?? null,
    // Compatibility aliases remain while callers migrate to the roadmap
    // contract's canonical pendingCount/nextPendingItem names.
    pendingItemCount: Number(entry?.pendingCount ?? entry?.pendingItemCount ?? 0),
    nextPendingItemText: String(entry?.nextPendingItem?.text ?? entry?.nextPendingItemText ?? ''),
    ...
    executionContract: entry?.executionContract ?? null,
...
export async function executeRoadmapDispatch(
  ...
  options?: {
    ...
    /**
     * H38-19 — which provider should run this. Omitted by default: the host
     * then applies the committed `dispatch.defaultTarget`, so the policy lives
     * in config rather than being restated by every caller. An unknown token is
     * refused by the route with a 400 rather than quietly defaulted.
     */
 ...
 */
/**
 * Release 3.5 milestone 1 — the reconciliation layer. Views render these
 * metric objects instead of computing portfolio numbers themselves; null
 * values are preserved (never coerced to 0) and render as an em dash.
 */
export async function getPortfolioSnapshot(): Promise<PortfolioSnapshot | null> {
  try {
    const res = await fetchJson<{ success: boolean; data: PortfolioSnapshot }>(`${API_BASE_URL}/portfolio/snapshot`);
