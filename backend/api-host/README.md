...
- `POST /api/ai/docs/improve/preview` generates a preview-only AI README/ROADMAP improvement (current vs proposed content, change summary, estimated score movement, warnings). Provider selection prefers a configured Anthropic/OpenAI key and degrades to a deterministic offline heuristic provider; no file is ever written by this route. Each preview appends a metadata record to `output/ai-doc-improvements/`. **No one-click egress (3.7 M4c):** when the selected provider is external, a request without `egressConfirmation: { providerId, file }` matching what the route would send returns `previewState: "ai-egress-confirmation-required"` with `egress` naming the provider, model and file, sends nothing, and writes no history. A repository listed in settings `ai.privateScopeRepos` (saved as `aiPrivateScopeRepos` through `POST /api/settings`) returns `previewState: "ai-egress-blocked"` with a `blockReason`, confirmation or not. The offline heuristic provider needs neither.
...
- `GET /api/providers` reports every configured provider with its capacity record, capacity verdict and detected availability. `supported` is a repository fact (a conforming adapter exists in this build); `installed`, `optedOut` and `command` are per-installation facts detected at runtime. `authenticated` is always `unknown` — proving an account works would spend its quota, so it is learned from the first real run.
- `POST /api/providers/:provider/opt-out` switches one provider off (`{"optOut": true}`) or back on for **this installation only**, and returns the refreshed availability. It writes `backend/config/installation.local.json` through `Get-InstallationStatePath` (honouring `REPO_MGMT_INSTALLATION_STATE_PATH`), never `settings.json` — that file is git-tracked, so recording one operator's machine state there would commit it on behalf of every installation. An unknown provider is refused with `400 validation`; a malformed state file is replaced rather than merged, and the response reports `stateReplaced`.
